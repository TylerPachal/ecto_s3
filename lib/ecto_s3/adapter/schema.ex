defmodule EctoS3.Adapter.Schema do
  ...
  def insert(adapter_meta, schema_meta, fields, on_conflict, returning, options) do
    ...
    %{source: _source, prefix: _prefix, schema: schema_module, context: context} = schema_meta
    ...
    path_format = Utils.get_option(:path_format, options, context, adapter_meta)
  ...
  def delete(adapter_meta, schema_meta, filters, options) do
    ...
    # TODO: Raise error if prefix is non-nil and point user towards :path_format

    IO.inspect(adapter_meta)
    ...
    path_format = Utils.get_option(:path_format, options, context, adapter_meta)
    path = Path.construct(schema_module, filters, format, path_format) |> IO.inspect(label: :delete_path)
